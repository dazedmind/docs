...
### When Adding New Features / Making Changes

Go to `/components/layout/Footer.tsx`

Then update the version

```text
Example: v.0.0.15 → v.0.0.16
```

Be sure to follow [Semantic Versioning](https://semver.org/) when updating the system version

<Note>
  Always update `CHANGELOG.md` file with new features following the [CHANGELOG Format](https://keepachangelog.com/en/1.1.0/)
</Note>
...
### Added
- Resend Integration: (IAM) New users now receive a welcome onboarding email upon account creation.
- Auth: Forgot password flow is now functional, including a dedicated reset password page.

### Changes
- Performance Optimization: Centralized data fetching per module - data is fetched upfront and cached, eliminating spinners and skeleton loaders on tab switches.
- Codebase: General refactoring for improved maintainability.
```
