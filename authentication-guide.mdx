...
Better Auth is configured in the application but the exact configuration file location is not specified in the repository structure shown. 

**[Inference]** The configuration is likely in:
- `lib/auth.ts` or
...
**[Inference]** Backup codes are likely generated when 2FA is first enabled, allowing users to access their account if they lose their authenticator device.
...
<AccordionGroup>
  ...
  <Accordion title="Rate Limiting" icon="gauge">
    **[Inference]** Login attempts are rate-limited to prevent brute force attacks
  </Accordion>
</AccordionGroup>
...
**[Inference]** Better Auth likely provides a React context for accessing auth state:
...
Common authentication issues and solutions can be found in the [Troubleshooting Guide](/troubleshooting#authentication-issues).

## Next Steps

<CardGroup cols={2}>
  <Card title="IAM Features" icon="users-gear" href="/features/iam">
    Learn about user management and access control
  </Card>
  <Card title="API Security" icon="shield" href="/guides/api-development">
