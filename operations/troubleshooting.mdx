...
- **Port 3000** — See [Getting started](/guides/getting-started) for PowerShell and bash commands to free a port.
...
Browser calls from another origin to RLink APIs must match **`ALLOWED_ORIGIN`** and cookie rules. Symptoms: preflight failures, or `401` with no session cookie. Align the origin with your deployment docs in [Deployment](/guides/deployment).
